    ...
    <form class="form-inline" enctype="multipart/form-data" action="{{ url('admin/config/update') }}" method="post">
        ...
        <table class="table table-striped table-bordered table-hover">
            <tr>
                <th>{{ __('Type') }}：</th>
                <td>
                    <input class="bjy-icheck" type="checkbox" name="164[]" value="local" @if(in_array('local', (array) $config['backup.backup.destination.disks'])) checked  @endif>{{ __('Local') }}
                    &emsp;
                    <input class="bjy-icheck" type="checkbox" name="164[]" value="oss" @if(in_array('oss', (array) $config['backup.backup.destination.disks'])) checked  @endif>{{ __('Aliyun') }} OSS
                </td>
            </tr>
            <tr>
