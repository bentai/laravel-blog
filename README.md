...
-    修改器
           ...
           进行查询时自动匹配修改器返回相关字段返回值


-    获取器

        需要动态调用当前方法，相当于只能在该model中使用的方法
...
-   remember 
        ...
        有时你可能想从缓存中找出一个数据，而当在请求的数据不存在时，
        程序能为你存储默认值。例如，你可能会想从缓存中取出所有用户，
        如果缓存中不存在用户数据时，就从数据库中将这些用户取出并放入缓存中
...
- 简单示例  验证get 请求返回状态码是否是200   assert Status
